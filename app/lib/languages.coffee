...
class Languages 
  ...
  constructor: ->
    _.each @LIST, (infos, lang) =>
      @LIST[lang].lang = lang

  get_infos: (lang) ->
    return @LIST[lang]

  get_color_byinfos: (infos, def) ->
    color = def 
    if not infos?
      return def
    ...
    if not infos.group? and not infos.group == infos.lang 
      return @get_color(infos.group, def)
    return def

  get_color: (lang, def) ->
    infos = Languages.get_infos(lang)
    return @get_color_byinfos(infos, def)

  get_color_byext: (ext, def) ->
    infos = @get_byextension(ext)
    return @get_color_byinfos(infos, def)
  
  get_byextension: (extension) ->
    name = extension.replace(".", "")
    extension = extension.toLowerCase()
    return _.find _.values(@LIST), (lang) ->
      if lang.primary_extension.toLowerCase() == extension 
        return true

      tt = _.map lang.extensions or [], (ext) ->
        return ext.toLowerCase()
      return _.contains( tt, extension ) or _.contains( lang.filenames, name )


  get_mode_byextension: (extension) ->
    lang = @get_byextension(extension)
    if lang?.ace_mode? 
      return lang.ace_mode
    else 
      return "text"

  get_autosuggestions: (query) ->
    query = query.toLowerCase()
    return _.reduce @LIST, (list, infos, language) ->
